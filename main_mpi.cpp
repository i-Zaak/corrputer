...
int run(int argc, char** argv)
{
    ...
    std::vector<std::string*> filesOut(corelComp->getOutsNumber());
    // Saver process has file output
    for(int i=0; i<corelComp->getOutsNumber(); i++)
    {
        std::stringstream ss;
        ss << "filename" << i;
        std::string outputFilename = config.Value(CONFIG_SECTION_OUTPUT, ss.str());
        filesOut[i] = new std::string(outputFilename);
    }
    ...
    // framework init
    framework = new DistributedComputationFramework(&fileIn, filesOut, corelComp);
    framework->open();
    ...
    if (mpiRank == 0) {
        ...
        MPI_COUT << "Output files: " <<std::endl;
        for(int i=0; i<framework->getOutputValues().size(); i++)
        {
            MPI_COUT << "\t" << *filesOut[i] << std::endl;
        }
        ...
        MPI_COUT << "Number of output streams: " << framework->getOutputValues().size() << std::endl;
        MPI_COUT << "Length of output streams:" << std::endl;
        for(int i=0; i<framework->getOutputValues().size(); i++)
        {
    ...
    if (mpiRank == 1) {
        for(int i=0; i<framework->getOutputValues().size(); i++)
        {
            std::string metadataOutFilename(*filesOut[i]);
            ...
            if (mIn.fail()) {
                ...
                mOut << "streamsCount = " << framework->getOutputValues()[i]->getStreamsCount() << std::endl;
                mOut << "streamsLength = " << framework->getOutputValues()[i]->getStreamsLength() << std::endl;
